...
index <- sample(1:21, size=50, replace=T)
...
### significant genes
```{r}
path <- file.path("outs/differential_expression",diffex_test, status)
#dir.create(path, recursive = T)
# matrix for differential expression
#designMat <- model.matrix(~ 0  + factor(visit) + factor(ID1) , data=colData(indata))
designMat <- model.matrix(~factor(visit) + factor(ID1) , data=colData(indata))
...
fitNut <- contrasts.fit(fit,contrast.matrixNut)
fitNut <- eBayes(fitNut)
limmaResNut <- topTable(fitNut, adjust.method = "BH",coef="Visit_6", n = Inf, sort.by = "P")


dim(limmaResNut[limmaResNut$adj.P.Val <0.7,])
dim(limmaResNut[limmaResNut$P.Value <0.05,])

table(colData(indata)$visit)
...
## Make a Heatmap of all genes
### gene matrix, scaled across genes (genes as columns)
mat = as.matrix(assay(indata[row.names(limmaResNut[limmaResNut$P.Value <0.05,]),],"log_counts_cpm"))
mat = t(scale(t(mat)))
...
df=data.frame(Helminth=colData(indata)$"HELMINTH",
              ...
              ID=colData(indata)$"subjid") %>% dplyr::arrange(Visit)

mat <- as.matrix(relocate(as.data.frame(mat), df$ID))

df <- df %>% select(c(Helminth, Nutrition, Visit, BMI))
...
```{r}
## save genes
write.csv(sig_limmaResNut, file.path(path, paste(diffex_test, status, "sig.csv", sep="_")))
write.csv(sig_limmaResNut2k, file.path(ipa, paste(diffex_test, status, "sig.csv", sep="_")))
...
### 2 All genes datatable and heatmap logFC-logFC {.tabset}
```{r }
#DT::datatable(sig_limmaResNut, 
#          options=list(scrollX=T,pAGELength=20),
#          rownames = T)
...
```{r}
## save genes
write.csv(sig_limmaResNut, file.path(path, paste0(diffex_test, status, "_sig.csv")))
#write.csv(limmaResNut[limmaResNut$P.Val <0.05,], file.path(path, paste0(status,"_pval_sig.csv")))
...
df=data.frame(Helminth=colData(indata)$"HELMINTH",
              ...
              BMI=colData(indata)$"BMI",
              ID1=colData(indata)$"ID1",
              ID=colData(indata)$"subjid") %>% dplyr::arrange(Visit, BMI, Nutrition)

mat <- as.matrix(relocate(as.data.frame(mat), df$ID))

df <- df %>% select(c(Helminth, Nutrition, Visit))
...
designMat <- model.matrix(~factor(visit)*factor(nutrition) + factor(ID1) + factor(HELMINTH), data=colData(indata))
...
colnames(designMat) <- gsub(pattern="factor\\(nutrition\\)", replacement =  "", colnames(designMat))
colnames(designMat) <- gsub(pattern="\\(Intercept\\)", replacement =  "Visit_1_malnourished", colnames(designMat))
colnames(designMat) <- gsub(pattern="Visit_6:well", replacement =  "Visit_6_well", colnames(designMat))
head(designMat)
...
contrast.matrixNut<- makeContrasts(Visit_6, levels = designMat)
contrast.matrixNut<- makeContrasts(Visit_6 - 2*well - Visit_6_well, levels = designMat)

fitNut <- contrasts.fit(fit,contrast.matrixNut)
fitNut <- eBayes(fitNut)
limmaResNut <- topTable(fitNut, adjust.method = "BH", n = "Visit_6 - Visit_1", sort.by = "P")


dim(limmaResNut[limmaResNut$adj.P.Val <0.05,])
dim(limmaResNut[limmaResNut$P.Val <0.001,])
table(colData(indata)$subjtype)
...
## Make a Heatmap of all genes
### gene matrix, scaled across genes (genes as columns)
mat = as.matrix(assay(indata[row.names(limmaResNut[limmaResNut$P.Val <0.001,]),],"log_counts_cpm"))
mat = t(scale(t(mat)))
...
df=data.frame(Helminth=colData(indata)$"HELMINTH",
              ...
              ID1=colData(indata)$"ID1",
              BMI=colData(indata)$"BMI",
              ID=colData(indata)$"subjid") %>% dplyr::arrange(Visit,Helminth )
