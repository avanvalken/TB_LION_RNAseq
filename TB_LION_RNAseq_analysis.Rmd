...
write.csv(sig_limmaResNut, file.path(path, "helminth_con_nutrition_sig.csv"))
...
g <- Heatmap(mat,show_row_names=F,show_column_names = F, top_annotation = ha, cluster_columns = T )
g
write.csv(sig_limmaResNut, file.path(path, "helminth_abs_vs_pres_well-only_sig.csv"))
...
write.csv(sig_limmaResNut, file.path(path, "mal_vs_well_con_helminth_sig.csv"))
...
g <- Heatmap(mat,show_row_names=F,show_column_names = F, top_annotation = ha, cluster_columns = T )
write.csv(sig_limmaResNut, file.path(path,"mal_v_well_helminthPos_sig.csv"))
# save plot
...
# save plot
write.csv(sig_limmaResNut, file.path(path,"mal_v_well_helminthNeg_sig.csv"))
# save plot
...
### Save
```{r}
ggsave(file.path(path, "umap_v1.svg"),plot=g, width = 3.5, height = 3.5, pointsize = 5)
ggsave(file.path(path, "umap_v1.png"),plot=g, width = 3.5, height = 3.5, pointsize = 5)
...
```{r}
indata.abs_mal <- se[,colData(se)$status=="absent_mal"] 
indata.abs_well <- se[,colData(se)$status=="absent_well"]
indata.pres_mal <- se[,colData(se)$status=="present_mal"]
indata.pres_well <- se[,colData(se)$status=="present_well"]
...
dir.create(path, recursive = T)
# matrix for differential expression
#designMat <- model.matrix(~factor(visit), data=colData(indata))
designMat <- model.matrix(~factor(ID1) + factor(visit), data=colData(indata))

designMat.2 <- model.matrix(~ 0 + factor(visit) + factor(ID1) , data=colData(indata))

design <- data.frame(Sample=colnames(assay(indata, "log_counts_cpm")),
                     Visit=colData(indata)$visit,
                     Pair=as.factor(colData(indata)$ID1))
#designMat <- model.matrix(~ 0 + factor(design$Visit) + factor(design$Pair))
...
colnames(designMat.2) <- gsub(pattern="factor\\(ID1\\)", replacement =  "", colnames(designMat.2))
colnames(designMat.2) <- gsub(pattern="factor\\(visit\\)", replacement =  "", colnames(designMat.2))

head(designMat)

fit <- lmFit(assay(indata, "log_counts_cpm"), designMat)
fit.2 <- lmFit(assay(indata, "log_counts_cpm"), designMat.2)
# Difference in expression between Failures and controls 

contrast.matrixNut<- makeContrasts(Visit_6, levels = designMat)
fitNut <- contrasts.fit(fit,contrast.matrixNut)
fitNut <- eBayes(fitNut)
limmaResNut <- topTable(fitNut, adjust.method = "BH", n = Inf, sort.by = "P")

contrast.matrixNut.2 <- makeContrasts(Visit_6, levels = designMat.2)
fitNut.2 <- contrasts.fit(fit.2,contrast.matrixNut.2)
fitNut.2 <- eBayes(fitNut.2)
limmaResNut.2 <- topTable(fitNut.2, adjust.method = "BH", n = Inf, sort.by = "P")

dim(limmaResNut[limmaResNut$adj.P.Val <0.05,])
dim(limmaResNut.2[limmaResNut.2$adj.P.Val <0.05,])
table(colData(indata)$subjtype)
...
## save genes
write.csv(sig_limmaResNut, file.path(path, paste0(status, "_sig.csv")))
write.csv(limmaResNut[limmaResNut$P.Val <0.05,], file.path(path, paste0(status, "_pval_sig.csv")))
...
### annotation data for heatmap
df=data.frame(Helminths=colData(indata)$"HELMINTH",
              Nutrition=colData(indata)$"nutrition",
              Visit=colData(indata)$"visit")


ha = HeatmapAnnotation(df = df, col = list(Nutrition=c("mal"="Red","well"="Blue"), Helminths=c("Present"="Black", "Absent"="Yellow"), Visit=c("Visit_1"="Grey", "Visit_6"="Turquoise")))
g <- Heatmap(mat,show_row_names=F,show_column_names = F, top_annotation = ha, cluster_columns = T )
g
...
colnames(designMat)
colnames(designMat) <- c("Intercept", "Visit_6")
...
# annotation
df=data.frame(Helminth=colData(se)$"HELMINTH",
              Nutrition=colData(se)$"nutrition") 


ha = HeatmapAnnotation(df = df, col = list(Nutrition=c("mal"="Red","well"="Blue"), Helminths=c("Present"="Black", "Absent"="Yellow")))

#png(filename = file.path("outs/metabolism/metab_heatmap.png"), width=2100, height = 2100)
pheatmap::pheatmap(all_pathway_score,cluster_cols = T,color = rev(mapal),scale = "row")
#dev.off()
```
