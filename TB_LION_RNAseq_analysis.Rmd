...
df=data.frame(Helminth=colData(indata)$"HELMINTH",
              ...
              ID=colData(indata)$"subjid") %>% dplyr::arrange(BMI, Nutrition, Visit, ID1)
...
df=data.frame(Helminth=colData(indata)$"HELMINTH",
              ...
              BMI=colData(indata)$"BMI",
              ID1=colData(indata)$"ID1",
              ID=colData(indata)$"subjid") %>% dplyr::arrange(Visit, BMI, Nutrition, ID1)
...
designMat <- model.matrix(~ 0  + factor(visit) + factor(ID1) + factor(HELMINTH), data=colData(indata))
...
df <- df %>% select(c(Helminth, BMI, Nutrition, Visit))
