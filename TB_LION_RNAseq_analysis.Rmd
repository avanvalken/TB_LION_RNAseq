...
contrast.matrixNut<- makeContrasts(Visit_6, levels = designMat)
contrast.matrixNut<- makeContrasts(Visit_6 - 2*mal - Visit_6_mal, levels = designMat)
...
pca_after <- pcaData[pcaData$Visit == "Visit_2", ]
...
ggplot(pcaData, aes(x=PC1, y=PC2)) +
  ...
  geom_segment(inherit.aes = F, 
               data=pca_pairs, 
               aes(x=PC1_before, y=PC2_before, xend=PC1_after, yend=PC2_after, group=Sample, color=Helminth_before), 
...
ggplot(pcaData, aes(x=PC1, y=PC2)) +
  ...
  geom_segment(inherit.aes = F, 
               data=pca_pairs, 
               aes(x=PC1_before, y=PC2_before, xend=PC1_after, yend=PC2_after, group=Sample), 
               arrow=arrow(length=unit(0.005,"cm")), alpha=0.4, linewidth=0.5) +  # Arrows for paired samples
...
ggsave(file.path(path, "euclidean_distance_PCA.png"), width=5, height=5)
ggsave(file.path(path, "euclidean_distance_PCA.svg"), width=5, height=5)
...
AsignatureBoxplot(ssgsea_res, name="ssGSEA", signatureColNames= names(samp_tbsigs),
                 annotationColName = c("nutrition_visit"), rotateLabels = T,scale = TRUE)
```
```{r}
