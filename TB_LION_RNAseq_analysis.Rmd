...
df=data.frame(Helminth=colData(indata)$"HELMINTH",
              ...
              status=colData(indata)$"status") %>% arrange( Nutrition, Helminth)
mat <- as.matrix(relocate(as.data.frame(mat), df$ID))mat <- as.matrix(rHelminthelocate(as.data.frame(mat), df$ID))
...
# save top 25 by logFC
write.csv(top_25_logfc, file.path(path, paste0(visit,"mal_vs_well_con_helminth_top_25_logfc.csv")))
write.csv(top_25_neg_logfc, file.path(path, paste0(visit,"mal_vs_well_con_helminth_top_25_neg_logfc.csv")))
...
## Make a Heatmap of all genes
### gene matrix, scaled across genes (genes as columns)
mat = as.matrix(assay(indata[row.names(limmaResNut[limmaResNut$adj.P.Val <0.05,]),],"log_counts_cpm"))
mat = t(scale(t(mat)))
...
# Perform PCA on transformed expression
...
# V1 vs V6 Boxplot of selected genes
```{r}
# path <- file.path("outs/differential_expression/v1_v_v6/all/visit_by_nutrition_interaction_plus_helminth.csv") are significant genes
# path to save plots
outs <- file.path("outs/differential_expression/v1_v_v6/boxplots_selected_genes")

# subset data on malnourished
indata <- se[,colData(se)$nutrition=="mal"]
# gene expression matrix
mat <- assay(indata, "log_counts_cpm", withDimnames=T)
assays(indata)

# selected genes
genes <- c("CD248", "C1QA", "SIGLEC1", "SERPING1", "BATF2", "ISG15")

# filter matrix
plot.data <- mat[which(rownames(mat) %in% genes),]
```

## Boxplot
```{r}
# melt plot.data; need visit, ID, gene, logCPM
df <- as.data.frame(t(plot.data))
df.1 <- df %>% 
               rownames_to_column(var="ID_visit") %>% 
               mutate(Visit=ifelse(grepl("visit_1", ignore.case = T, ID_visit), "Visit_1", "Visit_6")) %>% 
               mutate(ID=gsub("_VISIT_1|_VISIT_6", "" ,ID_visit)) %>% 
               select(-ID_visit)
melted <- reshape2::melt(df.1, vars=c(ID, Visit, logCPM)) #logCPM is value

ggplot(melted, mapping=aes(x=variable, y=value, fill=Visit)) +
               geom_boxplot()

```
### ggstats boxplots
```{r}
library(dlookr) # checks for normality; high p-values indicate normality
library(car) # checks for variance (leveneTest); low p value is different variances
library(ggstatsplot) # boxplots with stats

# test for normality with dlookr
norm.test <- melted %>% 
               group_by(Visit, variable) %>% 
               dlookr::normality(value) 
# non-normal for SIGLEC1, CD248, AND SERPING1, non-parametric
# variances differ
d <- melted %>% 
               group_by(Visit, variable)
car::leveneTest(value~variable,d) # Pr(>F) < 0.05; variances differ

# ggstatsplot::ggbetweenstats(data=melted,
#                x=c(variable, visit),
#                y=value,
#                type="nonparametric", 
#                var.equal=F)
...
ggstatsplot::grouped_ggwithinstats(
  data = melted,       # Your data frame
  x = Visit,  # The grouping variable (e.g., condition)
  y = value, # The dependent variable (e.g., value)
  grouping.var = variable,
  type="nonparametric", 
  var.equal=F,# The variable for splitting the analysis
  ylab="logCPM")
```

#### Save
```{r}
ggsave(file.path(outs, "selected_genes_mal_v1_v6.png"), width=14, height=7)
ggsave(file.path(outs, "selected_genes_mal_v1_v6.svg"), width=14, height=7)
...
sigboxes <- AsignatureBoxplot(ssgsea_res, name="ssGSEA",
                              signatureColNames= c("Darboe_RISK_11", "Leong_RISK_29"),
                               annotationColName = c("nutrition_visit"), 
                              rotateLabels = T,scale = TRUE) #+ 
              # geom_segment(inherit.aes = T, mapping=aes(visit, group=ID1, linetype= ID1))
sigboxes 
```


```{r}
ggsave(file.path("outs/TBSigProfiler", "nutriton_visit.svg"), width=8, height=4)
ggsave(file.path("outs/TBSigProfiler", "nutriton_visit.png"), width=8, height=4)
```
