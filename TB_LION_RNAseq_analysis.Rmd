...
write.csv(sig_limmaResNut, file.path(path, "helminth_plus_nutrition_sig.csv"))
...
# save plot
svg(file.path(path, "helminth_plus_nutrition_sig.svg"), width=7, height=7)
g
dev.off()
png(file.path(path, "helminth_plus_nutrition_sig.png"), width=7, height=7)
g
dev.off()
...
g
dev.off()
png(file.path(path, "helminth_abs_vs_pres_well-only_sig.png"), width=7, height=7)
g
dev.off()
...
png(file.path(path, "mal_vs_well_con_helminth_sig.png"), width=7, height=7)
g
dev.off()
g
```
...
g <- Heatmap(mat,show_row_names=T,show_column_names = F, top_annotation = ha, cluster_columns = T )
write.csv(sig_limmaResNut, file.path("mal_v_well_helminthPos_sig.csv"))
# save plot
svg(file.path(path, "mal_v_well_helminthPos_sig.svg"), width=7, height=7)
g
dev.off()
png(file.path(path, "mal_v_well_helminthPos_sig.png"), width=7, height=7)
g
dev.off()
...
# save plot
write.csv(sig_limmaResNut, file.path("mal_v_well_helminthNeg_sig.csv"))
# save plot
svg(file.path(path, "mal_v_well_helminthNeg_sig.svg"), width=7, height=7)
g
dev.off()
png(file.path(path, "mal_v_well_helminthNeg_sig.png"), width=7, height=7)
g
dev.off()
g
```
...
path <- file.path("outs/dimension_reduction")
#dir.create(path)
path1 <- file.path(path, "visit_6")
#dir.create(path1)
# filter by time
indata <- indata[,colData(indata)$visit=="Visit_6"] 
...
## Helminth vs no Helminth, control for nutrition {.tabset}
```{r }
path <- file.path("outs/differential_expression/visit_6")
#dir.create(path, recursive = T)
# matrix for differential expression
designMat <- model.matrix(~factor(HELMINTH) + factor(nutrition), data=colData(indata))
